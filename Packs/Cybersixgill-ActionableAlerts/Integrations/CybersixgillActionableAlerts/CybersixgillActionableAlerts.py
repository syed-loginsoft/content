...
def alerts_content_cached(sixgill_alerts_client, alert_id, organization_id):
    demisto.debug(f"Inside function: alerts_content_cached {alert_id=} , ddd ")
...
def get_alert_content(content_item, item_info, incident, sixgill_alerts_client):
    ...
    if cve_id:
        ...
        demisto.debug(f"before cached content call {alert_id=} , eeee ")
        for i in range(5):
            demisto.debug(f"calling cached function with {alert_id=} for {i=} time")
            content = alerts_content_cached(sixgill_alerts_client, item_info.get("id"), demisto.params().get('org_id', None))
        demisto.debug(f"After cached content call {alert_id=}, ffff ")
